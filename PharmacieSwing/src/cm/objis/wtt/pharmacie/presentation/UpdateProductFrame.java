...
public class UpdateProductFrame extends JFrame {
	 ...
	 */
	/**
	 * @param p est l'objet de la classe Produit à modifier
	 */
	public UpdateProductFrame(Produit p) {
		produit = p;
		...
		textFieldLibelle.setText(produit.getLibelle());
		
		textFieldPrix = new JTextField();
		textFieldPrix.setBounds(251, 135, 270, 33);
		contentPane.add(textFieldPrix);
		textFieldPrix.setColumns(10);
		textFieldPrix.setText(Double.toString(produit.getPrix()));
		...
		textFieldQuantite.setText(Integer.toString(produit.getQuantite()));
		...
		btnEnregistrer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(textFieldLibelle.getText().trim().equals("")) {
					...
					produit.setLibelle(textFieldLibelle.getText());
				}
				...
				try {
					...
					// j'enregistre les modification sur le produit en BD
					service.modifierProduitService(produit);
					JOptionPane.showMessageDialog(null, "Enregistrement effectué avec succès", "Info", 1);
					
					//J'affiche la fenêtre de listing
					ListProductFrame listFrame = new ListProductFrame();
					listFrame.setVisible(true);
					
					//Je ferme la fenêtre en cours
					dispose();
